          ...
          for(let i = indexOfQueen + 1; i < length;  i++) {
            ...
            if (this.field[way[i]].checker === 2) {
							...
							for (let j = i + 1; j < length; j++) {
								...
								if (this.field[way[j]].checker === 1 || this.field[way[j]].checker === 2) break;

								if (this.field[way[j]].checker === 0) {
									...
									const move = [hangman, newHangmanPlace, victim];
									intermediateMoves.push(move);

									//если втсретили пустое поле то проверяем диагональ этого поля на наличие вражиской пешки
									this.waysOfCells[way[j]].forEach(way2 => {

										// У ячейки максимум две диагонали, если мы не найдем в way2 ячейки [j-1] из way, то значит это нужная нам диагональ
										if (way2.indexOf(way[j-1]) === -1) {
											const lengthWay2 = way2.length;
											const indexOfEmptyCell = way2.indexOf(j);

											for (let k = indexOfEmptyCell + 1; k < lengthWay2; k++) {

												//Встречается ли на диагонали вражеская пешка, а за ней пустое поле. Нам достаточно такой проверки, остальные ячейки проверять не нужно.
							...
							//если за вражеской шашкой нет дополнительных срубов, то на все свободные клетки, можно встать
							if (countOfChops === 0 && intermediateMoves[0]) {
								this.field.chops = true;
								this.field.moves = this.field.moves.concat(intermediateMoves);
							}
